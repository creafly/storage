...
import (
	...
	"github.com/hexaend/storage/internal/i18n"
	"github.com/hexaend/storage/internal/infra/client"
)
...
func TenantValidatorMiddleware(identityClient *client.IdentityClient) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		valid, err := identityClient.ValidateTenantAccess(c.Request.Context(), tokenString, tenantID.(uuid.UUID), userID.(uuid.UUID))
		if err != nil || !valid {
