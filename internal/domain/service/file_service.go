...
func (s *FileService) GetByTenantID(ctx context.Context, tenantID uuid.UUID, fileType *entity.FileType, limit, offset int) (*entity.FileList, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 100 {
	...
	files, err := s.fileRepo.GetByTenantID(ctx, tenantID, fileType, limit, offset)
	if err != nil {
		return nil, err
	}

	total, err := s.fileRepo.CountByTenantID(ctx, tenantID, fileType)
	if err != nil {
		return nil, err
	}
...
func (s *FileService) DeleteMany(ctx context.Context, ids []uuid.UUID, tenantID uuid.UUID) (*BatchDeleteResult, error) {
	...
	for _, path := range pathsToDelete {
		if err := s.minioClient.Delete(ctx, path); err != nil {
		}
	}
