...
func TestFileRepository_GetByTenantID(t *testing.T) {
	...
	t.Run("returns files for tenant", func(t *testing.T) {
		...
		files, err := repo.GetByTenantID(ctx, tenantID, nil, nil, 10, 0, false)
		require.NoError(t, err)
	...
	t.Run("filters by file type", func(t *testing.T) {
		...
		files, err := repo.GetByTenantID(ctx, tenantID, nil, &imageType, 10, 0, false)
		require.NoError(t, err)
	...
	t.Run("respects limit and offset", func(t *testing.T) {
		...
		files, err := repo.GetByTenantID(ctx, tenantID, nil, nil, 2, 0, false)
		require.NoError(t, err)
		assert.Len(t, files, 2)

		files, err = repo.GetByTenantID(ctx, tenantID, nil, nil, 2, 2, false)
		require.NoError(t, err)
		assert.Len(t, files, 2)

		files, err = repo.GetByTenantID(ctx, tenantID, nil, nil, 2, 4, false)
		require.NoError(t, err)
		assert.Len(t, files, 1)
	})
