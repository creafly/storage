...
func (h *FileHandler) Upload(c *gin.Context) {
	...
	if folderIDStr := c.PostForm("folderId"); folderIDStr != "" {
		if parsedID, err := uuid.Parse(folderIDStr); err == nil {
			folderID = &parsedID
		}
	}
...
func (h *FileHandler) List(c *gin.Context) {
	...
	fileList, err := h.fileService.GetByTenantID(c.Request.Context(), tenantID.(uuid.UUID), folderID, fileType, limit, offset, includeAll)
	if err != nil {
...
func (h *FileHandler) GetUsage(c *gin.Context) {
	...
	usage, err := h.fileService.GetUsage(c.Request.Context(), tenantID.(uuid.UUID))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": messages.Errors.FetchFailed})
		return
	}
...
func (h *FileHandler) BatchDelete(c *gin.Context) {
	...
	result, err := h.fileService.DeleteMany(c.Request.Context(), req.IDs, tenantID.(uuid.UUID))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": messages.Errors.DeleteFailed})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"message": messages.Success.FileDeleted,
	})
}
